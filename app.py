...
St.caption("Paste any YouTube URL and get a strugtured summary in seconds.")

#Sidebar - API key
with st.sidebar:
  st.header("Settings")
  api_key = st.text_input("OpenAI API key", type="password", placeholder="")
  st.markdown("Get your key at [platform.openai.com](https://platform.openai.com/api-key)")

# UI
url = st.text_input("YouTube URL", placeholder="")
...
if st.button("Summarise", use_container_width= True):
  ...
  with st.spinner("Fetching transcript...."):
    try:
      transcript = get_trsnscript(video_id)
